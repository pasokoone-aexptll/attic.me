  ...
  return (
    ...
    >
      ...
      <div className="leading-8 text-[#ffffffbc] text-[1.15rem] copy-text">
        <p className="opacity-0">地球の午後にAWSは停止し</p>
        <p className="opacity-0">市場価値を株式で保存した会社はいずれ春の風の前の塵となる</p>
        <p className="opacity-0">ドメインは無職のカップラーメンの湯気と共に失効</p>
        <p className="opacity-0">この時代にアルゴリズムに評価されなかった文章は、誰にも見つからないままそれらと共に消えゆくのだろうか</p>
        ...
        <p className="opacity-0">attic は市場経済圏のインターネットから独立した異空間インターネットへ接続するための</p>
        <p className="opacity-0">モデムであり、ファイアーウォールです</p>
        <p className="opacity-0">attic は、プラットフォームではなく</p>
        <p className="opacity-0">保存形式、プロトコルになることを目指しています</p>
        <p className="opacity-0">そろそろ、夕暮れ時で5時になる。</p>

        <ProgressPercent localProgress={localProgress} label="こたつに潜る" />

        <Image className="target-image opacity-0 absolute bottom-20 left-210" src="/ohuda.png" alt="お札" width={200} height={45} priority />
      </div>
    </StageCopyLayout>
  );
}